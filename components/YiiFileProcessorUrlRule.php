...
class YiiFileProcessorUrlRule extends \CBaseUrlRule
{
	...
	{
		if(!preg_match('#^' . \fileProcessor\helpers\FPM::m()->cachedImagesBaseDir . '\/#', $pathInfo))
		{
			return false;
		}
		...
		{
			if(preg_match('#^' . \fileProcessor\helpers\FPM::m()->cachedImagesBaseDir . $pattern . '#', $pathInfo, $matches))
			{
