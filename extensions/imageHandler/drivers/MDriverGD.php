...
class MDriverGD extends MDriverAbstract
{
	 ...
	 *
	 * @return \fileProcessor\extensions\imageHandler\drivers\MDriverGD
	 */
	 ...
	 *
	 * @return \fileProcessor\extensions\imageHandler\drivers\MDriverGD|boolean
	 */
	public function watermark($watermarkFile, $offsetX, $offsetY, $corner = self::CORNER_RIGHT_BOTTOM)
	{
		...
		if ($wImg = $this->loadImage($watermarkFile)) {
			list($posX, $posY) = $this->getCornerPosition($corner, $wImg['width'], $wImg['height'], $offsetX, $offsetY);
	...
	public function rotate($degrees, $backgroundColor = array(0, 0, 0))
	{
		...
		$degrees = (int)$degrees;
		$backgroundColor = $this->getColor($this->_image, $backgroundColor);

		$this->_image = imagerotate($this->_image, $degrees, (int)$backgroundColor);
	...
	public function resizeCanvas($width, $height, $backgroundColor = array(255, 255, 255))
	{
		...
		$backgroundColor = $this->getColor($newImage, $backgroundColor);
	...
	protected function initImage($image = false)
	{
		...
		$this->preserveTransparency($this->_image);
